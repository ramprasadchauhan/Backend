...
const generateAccessAndRefreshToken = async (userId) => {
  try {
    ...
    throw new ApiError(
      500,
      "Somethig went wrong while genratinf refresh and access token"
    );
  }
};
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  if (existedUser) {
    throw new ApiError(409, "user with usename or email is already exixts");
  }
  ...
  const channel = await User.aggregate([
    ...
    },
    {
      $addFields: {
        ...
        isSubscribed: {
          $cond: {
            id: { $in: [req.user?._id, "$subscribers.subscriber"] },
            then: true,
            else: false,
          },
        },
